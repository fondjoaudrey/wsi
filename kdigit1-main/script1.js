...
function initCart() {
    ...
    if (cartToggle && cartContent) {
        ...
        document.addEventListener('click', (e) => {
            if (!cartToggle.contains(e.target) && !cartContent.contains(e.target)) {
    ...
    if (cartOrderBtn) {
        cartOrderBtn.addEventListener('click', () => {
            ...
            const whatsappUrl = `https://wa.me/237693404172?text=${encodeURIComponent(message)}`;
...
function updateCartDisplay() {
    ...
    if (cartItems) {
        ...
        if (cart.length === 0) {
            cartItems.innerHTML = '<p style="color: #9CA3AF; text-align: center; margin: 20px 0;">Votre panier est vide</p>';
        } else {
            cart.forEach(item => {
                const itemEl = document.createElement('div');
                itemEl.className = 'cart-item';
                itemEl.innerHTML = `
                    <span class="cart-item-name">${item.name} (${item.brand}) x${item.quantity}</span>
                    <span class="cart-item-price">${(item.price * item.quantity).toLocaleString()} FCFA</span>
    ...
    if (cartTotalEl) {
        cartTotalEl.textContent = cartTotal.toLocaleString() + ' FCFA';
    }
...
function generateWhatsAppMessage() {
    let message = 'Bonjour KDigit,\n\nJe souhaite commander les équipements suivants :\n\n';
    
    cart.forEach(item => {
        message += `• ${item.name} (${item.brand}) - Quantité: ${item.quantity} - Prix: ${(item.price * item.quantity).toLocaleString()} FCFA\n`;
    });
...
function saveCartToStorage() {
    localStorage.setItem('kdigit-cart', JSON.stringify(cart));
}

function loadCartFromStorage() {
    const savedCart = localStorage.getItem('kdigit-cart');
    if (savedCart) {
